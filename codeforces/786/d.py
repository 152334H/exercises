...
def solve():
    N = int(input())
    A = Ints()
    # the order of selection actually almost doesn't matter. 
    # the two steps allow you to swap the last two elements, the last [-4:-2] elements, and so on
    # (the first element is unswappable if N is odd)
    ls = []
    for i in range(N,0,-2):
        choices = A[max(0,i-2):i]
        if len(choices) == 1: ls.append(choices[0])
        else: ls += reversed(sorted(choices))
    for i in range(N-1):
        if ls[i] < ls[i+1]: return "NO"
    return "YES"
