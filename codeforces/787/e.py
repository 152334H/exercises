...
def solve():
    ...
    convert = {}
    lowerto = ord('a')
    for c in s: # c is a byte value!
        if not K: break
        if c in convert: continue
        moves = min(K,c-lowerto)
        target = c-moves
        reduced_chars = list(range(c, target-1, -1))
        if (rc := next((rc for rc in reduced_chars if rc in convert),None)):
            moves = c-rc 
            target = convert[rc]
        for rc in reduced_chars: convert[rc] = target
        K -= moves
    return ''.join(chr(convert.get(b,b)) for b in s)
