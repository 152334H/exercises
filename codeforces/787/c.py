...
'''The comments here are mostly nonsense. Please ignore.'''
def solve():
    s = '1'+input()+'0'
    # first, find the range of where the painting might've been dropped.
    prevOne, start, firstZero, end = 0,None,None,None
    ...
    1. The Thief is indeterminately within [start,end)
    2. The Thief revealed himself with the answers. Let's check for that.'''
    end = len(s)-1 if end is None else end
    if firstZero == len(s)-1: firstZero -= 1
    start = 1 if start == 0 else start
    #print(firstZero,start)
    '''
    if s[start] == '1' and s[end-1] == '0':
        # if s[start] was the thief, then there is no liar, and we want end = firstZero
        end = firstZero
    '''
    return firstZero-start+1
